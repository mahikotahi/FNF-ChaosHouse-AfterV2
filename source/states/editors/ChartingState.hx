...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	override function create()
	{
		...
		var eventIcon:FlxSprite = new FlxSprite(-GRID_SIZE - 5, -90).loadGraphic(Paths.image('chart_editor_shit/eventArrow'));
	...
	function addSongUI():Void
	{
		...
		var clear_events:FlxButton = new FlxButton(320, 310 - 30, 'Clear events', function()
		{
		...
		var clearJSON:FlxButton = new FlxButton(320, clear_notes.y + 30, 'Clear JSON', function()
		{
			openSubState(new Prompt('This action will clear current progress.\n\nProceed?', 0, function()
			{
				_song.notes = [];
		...
		clearJSON.color = FlxColor.RED;
		clearJSON.label.color = FlxColor.WHITE;
		...
		tab_group_song.add(new FlxText(stepperBPM.x + 100, stepperBPM.y, 0, 'Song Offset:'));
	...
	function setupNoteData(i:Array<Dynamic>, isNextSection:Bool):Note
	{
		...
		{ // Event note
			...
			var hardCodedEventImages:Array<String> = ['Camera Section', 'Camera Flash'];

			eventImage(note, 'Camera Section', 'CameraEvent');
			eventImage(note, 'Camera Flash', 'CameraFlash');

			if (!hardCodedEventImages.contains(note.eventName))
			{ // Softcoding
				try
				{
					try
	...
	function eventImage(note:Note, eventName:String = '', eventImage:String = '')
	{
		if (note.eventName == eventName)
			note.loadGraphic(Paths.image('chart_editor_shit/${eventImage}'));
	}
